...
\DIFaddbegin \begin{figure*}[htb]
	\centering
	%DIF > 	\includegraphics[width=0.9\columnwidth]{SchemaExos}
	\def\svgwidth{2\columnwidth}
	\begin{footnotesize}
		\input{imgRevised/rehab_description.pdf_tex}
	\end{footnotesize}
	\caption{\DIFaddFL{(a) The Rehab-Exos. It is a 5 DOF upper-limb exoskeleton  with 4 actuated joints. The joints $J_1$, $J_2$ and $J_4$ share the same characteristics: high reduction ratio (100:1) by means of harmonic drive, embedded torque sensor and maximum actuation torque of 150 }\ \DIFaddFL{Nm. The joint $J_3$ is composed by a semi-circular guide actuated by a DC motor through tendon transmission. Joint $J_5$ is passive and the exoskeleton is equipped of a force/torque sensor at the end-effector that is used for evaluation purposes. (b) A schematic representation of the Rehab-Exos exoskeleton. (c) CAD section of the $J_1$, $J_2$ and $J_4$ joint actuator of the Rehab-Exos. (d) Characteristic dimensions of the torque sensor.}}
	\label{fig:rehabexosSchema}
\end{figure*}

\DIFaddend The Rehab-Exos is an active robotic exoskeleton (\figurename \ \DIFdelbegin \DIFdel{\ref{fig:rehabexos1})  which is }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:rehabexosSchema})  }\DIFaddend designed with the idea to be \DIFdelbegin \DIFdel{compact}\DIFdelend \DIFaddbegin \DIFadd{modular}\DIFaddend , easily reconfigurable and \DIFdelbegin \DIFdel{to have }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend a good trade-off between transparency and force \DIFdelbegin \DIFdel{replication}\DIFdelend \DIFaddbegin \DIFadd{payload}\DIFaddend .
It was conceived for rehabilitation applications and it is designed in such a way to generate controlled contact forces/torques not only at its \DIFdelbegin \DIFdel{end-link }\DIFdelend \DIFaddbegin \DIFadd{end-effector }\DIFaddend handle, but also at  intermediate \DIFdelbegin \DIFdel{links}\DIFdelend \DIFaddbegin \DIFadd{contact points with the user arm}\DIFaddend . When the user is wearing the device he can control the full force interaction with the exoskeleton and guide/be guided involving \DIFdelbegin \DIFdel{all }\DIFdelend \DIFaddbegin \DIFadd{shoulder and elbow) }\DIFaddend articulations of the arm\DIFdelbegin \DIFdel{(wrist, shoulder, elbow)}\DIFdelend . 
The physical interaction between user and exoskeleton is monitored by the joint torque sensors which \DIFdelbegin \DIFdel{performances depend }\DIFdelend \DIFaddbegin \DIFadd{performance depends }\DIFaddend on several design and implementation aspects that are addressed in subsection \ref{subsec:DesignTorqueSensor}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Mechanical design of the Rehab-Exos} 
%\hldone{Done}
\label{subsec:mechanicalDesign}
As depicted  in \figurename \ \DIFdelbegin \DIFdel{\ref{fig:rehabexos1} and in }%DIFDELCMD < \figurename \ %%%
\DIFdelend \ref{fig:rehabexosSchema}, the exoskeleton has a serial architecture isomorphic with the human kinematics that comprises: a shoulder joint  fixed in space and composed by three active joints $J_1$, $J_2$ and $J_3$; an active elbow joint $J_4$; and a passive revolute joint $J_5$ allowing  \DIFdelbegin \DIFdel{for }\DIFdelend wrist prono/supination.  For a more detailed description of both Rehab-Exos and actuation groups, the reader can refer to \cite{vertechy2009development}.
%\begin{figure}[htb]
%	\centering
...
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=0.9\columnwidth]{SchemaExos}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{A schematic representation of the Rehab-Exos exoskeleton.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:rehabexosSchema}
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=0.7\columnwidth]{JointActuator}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{CAD section of the $J_1$, $J_2$ and $J_4$ joint actuator of the Rehab-Exos.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:exosActuatorCAD}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend %DIF > \begin{figure}[]
%DIF > 	\centering
%DIF > 	\includegraphics[width=0.7\columnwidth]{JointActuator}
%DIF > 	\caption{CAD section of the $J_1$, $J_2$ and $J_4$ joint actuator of the Rehab-Exos.}
%DIF > 	\label{fig:exosActuatorCAD}
%DIF > \end{figure}
%
\par The three joints $J_1$, $J_2$ and $J_4$ of the exoskeleton are motorized through identical actuation groups. Each joint features a custom-made frame-less brush-less torque motor integrating a compact Harmonic Drive (HD) component set. The actuator provides a joint output torque equal to $150\ Nm$ with an overall weight equal to $3.7\ Kg$ and a motor shaft inertia reduced to the joint output shaft \DIFdelbegin \DIFdel{$Jm = 3.7\ Kgm^2$}\DIFdelend \DIFaddbegin \DIFadd{$Jm = 2.5 \ Kgm^2$}\DIFaddend . The Harmonic Drive performs a reduction equal to 100:1. Due to the adopted mechanical components, the joints feature limited back-drivability at motor power-off and limited mechanical complexity to ease maintenance as well as reduce costs. A CAD section of the $J_1$, $J_2$ and $J_4$ joints is depicted in \figurename \ \DIFdelbegin \DIFdel{\ref{fig:exosActuatorCAD}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:rehabexosSchema} c)}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=0.7\columnwidth]{sensorDimensions}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Characteristic dimensions of the torque sensor.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:sensorDimensions}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend %DIF > \begin{figure}[]
%DIF > 	\centering
%DIF > 	\includegraphics[width=0.7\columnwidth]{sensorDimensions}
%DIF > 	\caption{Characteristic dimensions of the torque sensor.}
%DIF > 	\label{fig:sensorDimensions}
%DIF > \end{figure}
%
...
The axial torsional stiffness of the sensor is \DIFdelbegin \DIFdel{$k_s = 30 \ kNm/rad$ }\DIFdelend \DIFaddbegin \DIFadd{$k_s = 30$  kNm/rad }\DIFaddend and can be calculated as in \eqref{eqn:sensorStiffness}.
\begin{equation}
...
Q = \frac{3r+L}{6r+3L}	
\end{equation}
where, according to \figurename \ \DIFdelbegin \DIFdel{\ref{fig:sensorDimensions}, $R$ is the radius of the external sensor ring, }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:rehabexosSchema} d), %DIF > $R$ is the radius of the external sensor ring, 
}\DIFaddend $r$ is the radius of the internal sensor ring, $L$ is length of the beams and $a$ is the side length of the beam section.
The characteristic dimensions of the sensor are reported in \tablename \ \ref{tab:sensorDimension}.
Moreover, the overall joint torsional stiffness reduced to the joint output shaft is \DIFdelbegin \DIFdel{$k = 11.38\ kNm/rad$}\DIFdelend \DIFaddbegin \DIFadd{$k = 11.38\ $kNm/rad}\DIFaddend .
\par The position of the strain gauges on the beam is a trade-off: if they were positioned in the middle of the beam the sensor sensitivity would be low, on the contrary, if they were positioned near the extremities the sensor reads would be affected by the non-linearities of the rounds of the beam. The selected distance from the extremities was \DIFdelbegin \DIFdel{$p = 1/8 \ L = 3 \ mm$}\DIFdelend \DIFaddbegin \DIFadd{$p = 1/8 \ L = 3$ mm}\DIFaddend .
...
\par Theoretically, i.e. using \eqref{eq:strainEq}, at \DIFdelbegin \DIFdel{$3 \ mm$ }\DIFdelend \DIFaddbegin \DIFadd{$3 \ $mm  }\DIFaddend from the inner ring and under an axial torque of \DIFdelbegin \DIFdel{$120 Nm$}\DIFdelend \DIFaddbegin \DIFadd{120 Nm}\DIFaddend ,  a maximum strain of \DIFdelbegin \DIFdel{$2.7*10^-3 \ m$ }\DIFdelend \DIFaddbegin \DIFadd{$2.7\times 10^-3 $ m }\DIFaddend is obtained.
The same test has been conducted using a FEM software tool (Ansys\textregistered) because the surface of the strain gauge is not negligible compared to the beam one (see \figurename \ \ref{fig:strainGaugeDeformation}) obtaining a maximum strain of \DIFdelbegin \DIFdel{$1.98*10^-3 \ m$}\DIFdelend \DIFaddbegin \DIFadd{$1.98 \times 10^-3 $m}\DIFaddend .
The strain of each strain gauge when a \DIFdelbegin \DIFdel{$1 \ Nm$ }\DIFdelend \DIFaddbegin \DIFadd{$1 $Nm }\DIFaddend load is applied can be shown in \tablename \ \ref{tab:sensorStrain}.
\begin{table}[]
	\renewcommand{\arraystretch}{1.3}
...
\end{equation}
where $k_s$ is a scaling factor equal to $7.87e^{-3}$, $k_{ex}$ is the sensitivity to linear mounting misalignment  equal to $3$, $k_{e\theta}$ is the sensitivity to angular mounting misalignment equal to $2.3$, whereas $e_x$ and $e_{\theta}$ are the positional and angular misalignment errors respectively (see \figurename \ \DIFdelbegin \DIFdel{\ref{fig:strainGaugeMisalignment})}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:strainGaugeDeformation} a))}\DIFaddend . Equation \eqref{eq:sensitivityMisalignment} and the measured sensitivity of 0.067 lead to a misalignment errors of millimeters and decades of degree, but these values are over the actual misalignment the installation operator may have introduced as \DIFaddbegin \DIFadd{it }\DIFaddend can be seen in \figurename \ \DIFdelbegin \DIFdel{\ref{fig:strainGaugeParticular}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:strainGaugeDeformation} b)}\DIFaddend .
%
\DIFdelbegin %DIFDELCMD < \begin{figure}[]
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \begin{figure*}[]
	\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \subfigure[]{\includegraphics[height=0.35\columnwidth]{erroreMontaggioSensore} \label{fig:strainGaugeMisalignment}}
%DIFDELCMD < 	\subfigure[]{\includegraphics[height=0.32\columnwidth]{strainGaugeParticular.jpg} \label{fig:strainGaugeParticular}}
%DIFDELCMD < 	%%%
\DIFdelendFL %DIF > 	\subfigure[]{\includegraphics[height=0.35\columnwidth]{erroreMontaggioSensore} \label{fig:strainGaugeMisalignment}}
%DIF > 	\subfigure[]{\includegraphics[height=0.32\columnwidth]{strainGaugeParticular.jpg} \label{fig:strainGaugeParticular}}
	\DIFaddbeginFL \def\svgwidth{2\columnwidth}
	\begin{footnotesize}
		\input{imgRevised/fem_sensore.pdf_tex}
	\end{footnotesize}
	\DIFaddendFL \caption{A possible cause of high sensitivity to non-axial load is the strain gauges's mounting misalignment. The linear displacement $e_x$ and the angular one $e_{\theta}$ are highlighted in (a). A detail of the mounted strain gauges on the torque sensor in (b). \DIFaddbeginFL \DIFaddFL{(c) For the FEM analysis a more dense grid mesh for the zone of interest has been used. For each area the average strain along the radial direction has been computed. (d) The FEM analysis results of the torque sensor, and of the flexible spline  deformation under non-axial load (e).}\DIFaddendFL }
	\DIFdelbeginFL %DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:strainGaugeDeformation}
\end{figure*}
\DIFaddend 
...
\DIFdelend \DIFaddbegin \begin{figure}[b]
	\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \subfigure[]{\includegraphics[width=0.49\columnwidth]{schemaGiuntoMod}\label{fig:schemaLinkGiuntoSens}}
%DIFDELCMD < 	\subfigure[]{\includegraphics[width=0.49\columnwidth]{schemaMolle.pdf}\label{fig:schemaMolle}}
%DIFDELCMD < 	%%%
\DIFdelendFL %DIF > 	\subfigure[]{\includegraphics[width=0.49\columnwidth]{schemaGiuntoMod}\label{fig:schemaLinkGiuntoSens}}
%DIF > \subfigure[]{\includegraphics[width=0.49\columnwidth]{schemaMolle.pdf}\label{fig:schemaMolle}}
...
\end{figure}
%
\par For the study of the hyper-static system a linear  elastic behavior of the system parts were supposed and the system response at non-axial moments was modeled as a mono-dimensional model\DIFdelbegin \DIFdel{as in }%DIFDELCMD < \figurename %%%
\DIFdel{\ref{fig:schemaMolle}. }\DIFdelend \DIFaddbegin \DIFadd{. %DIF > % as in \figurename \ref{fig:schemaMolle}.
}\DIFaddend The overall joint stiffness to non-axial moments $K_{TOT}$ was experimentally evaluated, whereas the non-axial moment stiffness of the torque sensor $K_{TS}$ and of the HD $K_{HD}$ were computed via FEM analysis. The FEM results are depicted in \figurename \ \ref{fig:strainGaugeDeformation} and the stiffness values are reported in table \tablename \ \ref{tab:nonAxialStiffness}.
%
\DIFdelbegin %DIFDELCMD < \begin{figure}[]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=0.7\columnwidth]{strainGaugeDeformation} 
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{For the FEM analysis a more dense grid mesh for the zone of interest has been used. For each area the average strain along the radial direction has been computed.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:strainGaugeDeformation}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend %DIF > \begin{figure}[]
%DIF > 	\centering
%DIF > 	\includegraphics[width=0.7\columnwidth]{strainGaugeDeformation} 
%DIF > 	\caption{For the FEM analysis a more dense grid mesh for the zone of interest has been used. For each area the average strain along the radial direction has been computed.}
%DIF > 	\label{fig:strainGaugeDeformation}
%DIF > \end{figure}
%
\begin{table}[]
	\renewcommand{\arraystretch}{1.3}
...
%
%
\par A possible mounting misalignment of the hyper-static chain may be a collinear and/or concentric mounting misalignment between the sensor axis and the bearing axis. In this case, the HD works as an universal joint that connect the sensor (that is connected to the \DIFdelbegin \DIFdel{(}\DIFdelend n+1 \DIFdelbegin \DIFdel{)-th }\DIFdelend link ) and the n-th link. The sensitivity to non-axial moments defined in equation \eqref{eq:sensitivityToNonAxLoad} and  the mechanical properties  in \tablename \ \ref{tab:nonAxialStiffness} lead to a theoretical mounting misalignment of about $0.5 \ mm$, but this value is not in agreement with the design tolerances and components data-sheets from which a misalignment of about $0.05 \ mm$ results in the worst case.
%
...
\par In order to minimize this undesired effect\DIFdelbegin \DIFdel{the authors propose a  }\DIFdelend \DIFaddbegin \DIFadd{, we adopted a  model-free adaptive }\DIFaddend method based on artificial neural networks (ANN) to characterize and compensate this non-linear \DIFdelbegin \DIFdel{behavior that results difficult to model. 
}\DIFdelend \DIFaddbegin \DIFadd{response of the sensors, in alternative to modeling approaches in reason of the complexity of the phenomenon. 
%DIF >  that might be difficult to model.
}\DIFaddend %
%
Considering the ideal and linear \DIFaddbegin \DIFadd{response of the }\DIFaddend sensor, the torque readings can be expressed as $\tau_s = k_v * v$, where $v$ is the measured voltage tension and $k_v$ is the torque sensor's voltage constant. In the real case it be can written \DIFdelbegin \DIFdel{$\hat{\tau}_s = k_v * v + \delta\tau$}\DIFdelend \DIFaddbegin \DIFadd{$\hat{\tau}_s = k_v \cdot v + \delta\tau$}\DIFaddend , where $\delta\tau$ is the non-linear influence on the sensor readings due to the mounting and non-axial loads.
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\subfigure[]{\includegraphics[height=0.4\columnwidth]{sensoreCarichiSpuri} \label{fig:sensoreCarichiSpuri}	}
%DIFDELCMD < 	\subfigure[]{\includegraphics[height=0.4\columnwidth]{HDCarichiSpuri} \label{fig:HDCarichiSpuri} }
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{The FEM analysis results of the torque sensor (a) and of the flexible spline (b) deformation under non-axial load.}}
%DIFAUXCMD
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend %DIF > \begin{figure}[]
%DIF > 	\centering
%DIF > 	\subfigure[]{\includegraphics[height=0.4\columnwidth]{sensoreCarichiSpuri} \label{fig:sensoreCarichiSpuri}	}
%DIF > 	\subfigure[]{\includegraphics[height=0.4\columnwidth]{HDCarichiSpuri} \label{fig:HDCarichiSpuri} }
%DIF > 	\caption{The FEM analysis results of the torque sensor (a) and of the flexible spline (b) deformation under non-axial load.}
%DIF > \end{figure}
%
%
For all the three sensors the $k_v$ constants have been experimentally evaluated. In order to minimize the effect of non-linear undesired term $\delta\tau$,  an ANN has been used. ANN is a mathematical approximation approach that is capable \DIFdelbegin \DIFdel{to infer }\DIFdelend \DIFaddbegin \DIFadd{of inferring }\DIFaddend non-linear behavior from experimental acquisitions. The ANN with 7 neurons in the hidden layer and sigmoid activation function are \DIFdelbegin \DIFdel{deputed }\DIFdelend \DIFaddbegin \DIFadd{employed }\DIFaddend to estimate the error on the basis of the 4 angles and load on each axis. The angular information is useful to infer the assembly error component, whereas for the load influence the gravitational torque has been used. 
...
The actual sensor torque estimation is given by $\bar{\tau}_s = k_v * v + \delta\tau(\vect{\theta_m},G_i)$\DIFdelbegin \DIFdel{, and a scheme of this estimation is shown in the }%DIFDELCMD < \figurename \ %%%
\DIFdel{\ref{fig:NN_schema}.}%DIFDELCMD < \begin{figure}[]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=0.7\columnwidth]{torqueEstimation}	
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{The estimated sensor torque is obtained as the sum of sensor's reading and the predicted undesired non-linear term.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:NN_schema}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.%DIF > , and a scheme of this estimation is shown in the \figurename \ \ref{fig:NN_schema}.
%DIF > \begin{figure}[]
%DIF > 	\centering
%DIF > 	\includegraphics[width=0.7\columnwidth]{torqueEstimation}	
%DIF > 	\caption{The estimated sensor torque is obtained as the sum of sensor's reading and the predicted undesired non-linear term.}
%DIF > 	\label{fig:NN_schema}
%DIF > \end{figure}
}\DIFaddend %\hl{QUi} As can be shown in the \figurename \ N, the introduction of the undesired term prediction induce a reduction in interaction force while a zero torque following control is implemented (see section N). Note that the force reduction is measured by the end effector force/torque sensor, while the  torque control use only the estimated torque using the sensor readings.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
